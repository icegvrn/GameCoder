...
function game.load()
    ...
    for l = 1, game.MAPHEIGHT do
        game.fog[l] = {}
        for n = 1, game.MAPWIDTH do
            game.fog[l][n] = 1
        end
    end
    hero.load(game)
...
function game.draw()
    ...
    for n = 1, #game.fog do
        for i = 1, #game.fog[n] do
            love.graphics.setColor(0, 0, 0, game.fog[n][i])
...
function game.mapClearFog(pLine, pCol)
    for l = pLine - 1, pLine + 1 do
        for c = pCol - 1, pCol + 1 do
            if c > 0 and c <= game.MAPWIDTH and l > 0 and l <= game.MAPHEIGHT then
                game.fog[l][c] = 0
            end
        end
    end
...
function game.mapClearFogProgress(pLine, pCol)
    for l = 1, game.MAPHEIGHT do
        for c = 1, game.MAPWIDTH do
            local dist = math.dist(c, l, pCol, pLine)
            if dist < 4 then
                if game.fog[l][c] > dist * 0.2 then
                    game.fog[l][c] = dist * 0.2
                end
            end
        end
    end
